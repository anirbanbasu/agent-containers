...
apt/Node.js/`uv` baseline, `packages-apt.txt`/`packages-uv.txt`, AdaL
...
On a host shared by multiple accounts, tag the image per user (`adal:alice`,
`adal:bob`) rather than overwriting a shared `adal` tag — and use that same
...
image's system Python instead. No credentials, extensions, skills, plugins,
