...
docker build --build-context shared=agent-images/shared \
  ...
  -t kilo-code:$(whoami) agent-images/kilo-code
```

`Dockerfile` splits into a `base` stage that installs everything —
apt/Node.js/`uv` baseline, `packages-apt.txt`/`packages-uv.txt`, Kilo CLI
...
(`kilo-code:alice`, `kilo-code:bob`) rather than overwriting a shared
`kilo-code` tag — and use that same tag, plus a per-user home volume
