...
- **Configuration can quietly opt back out.** Setting the egress allowlist to allow everything disables the firewall outright, and an overly broad allowlist entry — a whole domain when one subdomain was meant — weakens it without looking like a mistake. The strength of this design is a property of how it's configured, not something the image guarantees on its own regardless of the flags it's started with. In particular, `AGENT_ALLOWED_EGRESS=*` (the unrestricted-network profile) doesn't just remove the allowlist — it lets the container reach whatever host/VM-side network services happen to be reachable on its network path (e.g. a Docker Desktop VM's own SSH or RPC ports), not only the public internet. That's not a container escape on its own, but it is real reachability the egress-allowlisted profile would otherwise deny.

## Why not just a virtual machine or a microVM?

A virtual machine or microVM is a perfectly valid containment strategy, and in fact, the principles outlined above can be applied to them as well. However, they come with their own trade-offs in terms of resource usage, startup time, and complexity. Containers offer a lightweight and efficient way to achieve isolation while still allowing for flexibility and ease of use. The choice between containers and VMs ultimately depends on the specific requirements of the project and the level of isolation needed. An additional thing to keep in mind with a micro VM such as [Docker Sandboxes](https://docs.docker.com/ai/sandboxes/) is licensing and the need for a cloud-managed user account.