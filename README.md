...
- **`aider`** — hardened container for [Aider](https://aider.chat/), the git-native terminal pair-programming agent. See `docs/container-images/aider.md`.
...
- **`volume-bridge`** — a loopback-only, read-only SFTP sidecar that exposes selected named agent-home volumes to host tools through SSHFS without granting them Docker access. See `docs/container-images/01-volume-bridge.md`.
