...
| Clone | Require explicit retain-or-skip selections for credential references and encrypted credentials, and the necessary source and destination key sources. Unresolved choices fail with actionable instructions. Destination key scope remains per-user or per-profile, not per-credential. |
